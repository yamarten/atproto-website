...
ATプロトコルは、オープンウェブそのものをモデルにした「小さな世界のフォールバックを伴う大きな世界」という設計です。ウェブでは、個々のコンピューターがコンテンツをネットワークにアップロードし、そのすべてのコンテンツが他のコンピューターへブロードキャストされます。同様に、ATプロトコルでは、少数の大規模アグリゲーターにメッセージを送り、ネットワーク全体のパーソナルデータサーバーへデータをブロードキャストします。
...
モデレーションはスタックの2箇所で処理されます。[*Osprey*](https://github.com/bluesky-social/osprey-atproto)はイベントストリームの判定エンジンと分析用UIで、自動的な調査と措置の実行を目的としています。[*Ozone*](https://github.com/bluesky-social/ozone)はラベリングサービスとモデレーション判断のためのウェブフロントエンドです。

OspreyはPythonで書かれており、Discordとの協働で設計・オープンソース化されました。Ospreyは人間が記述したルールを通じてアクションを処理し、ラベルを発行したりAPIへのwebhookを出力したりするためのPythonライブラリです。構造化されたロジック、ユーザー定義関数、外部シグナルを用いてイベントを評価し、ラベル・判定・アクションを割り当てます。これらの一部、例えば毒性検出のToxrankモデルは、Osprey向けに独自のウェブエンドポイントを公開するファインチューニング済みLLMやその他の分類器を利用しています。私たちは画像OCRとハッシュ化も活用して、実行可能なモデレーションメタデータを作成し、それを自動的にアクション化したり、Ozoneを通じてモデレーターに提示したり、結果的にDiscoverフィードのアルゴリズム形成に役立てたりしています。

内部のヒューリスティックの一部は非公開ですが、OspreyとOzoneは、Bluesky Lexiconと機能セットを再実装する場合でも、別種のAtmosphereアプリとして異なるコンテンツセットで運用する場合でも、独自のルールとモデレーション運用を持つ他のアプリホストが導入・運用できるように設計されています。
...
- 私たちが日常的に使うコミュニティツールも多数あります。例えば、Rustで書かれた[Microcosm](https://www.microcosm.blue/)のツール群は、専用のAppViewを必要とせずにクエリ層を構築するためのAPI群を提供します。詳細はhttps://constellation.microcosm.blue/を参照してください。
